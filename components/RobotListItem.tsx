...
import { Robot, useRobotsStore } from '../store/robotStore';
...
export const RobotListItem: React.FC<Props> = ({ robot }) => {
  ...
  const onDelete = () => {
    Alert.alert('Supprimer', `Supprimer ${robot.name} ?`, [
      ...
      { text: 'Supprimer', style: 'destructive', onPress: () => remove(robot.id) }
    ]);
  };

  const goEdit = () => router.push({ pathname: '/(main)/tp4A-robots/edit/[id]', params: { id: robot.id } });
